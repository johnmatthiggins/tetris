...
tetrominoes = {
    "long": Tetromino(
        color="blue", shape=((O, O, O, O), (X, X, X, X), (O, O, O, O), (O, O, O, O))
    ),
    "square": Tetromino(color="yellow", shape=((X, X), (X, X))),
    "hat": Tetromino(color="pink", shape=((O, X, O), (X, X, X), (O, O, O))),
    "right_snake": Tetromino(color="green", shape=((O, X, X), (X, X, O), (O, O, O))),
    "left_snake": Tetromino(color="red", shape=((X, X, O), (O, X, X), (O, O, O))),
    "left_gun": Tetromino(color="cyan", shape=((X, O, O), (X, X, X), (O, O, O))),
    "right_gun": Tetromino(color="orange", shape=((O, O, X), (X, X, X), (O, O, O))),
}
...
def shape_str(shape):
    ...
    return "\n".join("".join(map({"X": "X", None: "O"}.get, line)) for line in shape)
...
def test():
    ...
    assert (
        rotate(tetrominoes["right_snake"].shape, 4) == tetrominoes["right_snake"].shape
    )

    assert rotate(tetrominoes["hat"].shape) == ((O, X, O), (O, X, X), (O, X, O))

    assert rotate(tetrominoes["hat"].shape, 2) == ((O, O, O), (X, X, X), (O, X, O))
